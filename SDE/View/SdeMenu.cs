...
namespace SDE.View
{
    public partial class SdeEditor : TkWindow {
		...
		private void _menuItemDatabaseSave_Click(object sender, RoutedEventArgs e) {
			try {
				new GrfThread(_save, this, 200, null, false, true);
			}
			catch (Exception err) {
				ErrorHandler.HandleException(err);
			}
		...
		private void _export(ServerType mode, string subPath, FileType fileType = FileType.Detect) {
			...
			try {
				...
				if (path != null) {
					new GrfThread(() => _internalExport(mode, path, subPath, fileType), this, 200, null, false, true);
				}
			}
			catch (Exception err) {
				ErrorHandler.HandleException(err);
		...
		private void _menuItemConvertClientDbToLua_Click(object sender, RoutedEventArgs e) {
			try {
				new GrfThread(() => _clientDbExport(FileType.Lua), this, 200, null, true);
			}
			catch (Exception err) {
				ErrorHandler.HandleException(err);
			}
		...
		private void _menuItemConvertClientDbToTxt_Click(object sender, RoutedEventArgs e) {
			try {
				new GrfThread(() => _clientDbExport(FileType.Txt), this, 200, null, true);
			}
			catch (Exception err) {
				ErrorHandler.HandleException(err);
			}
